...
allfiles := $(shell { echo $a.txt; grep -Ev '^[ 	]*(;|#|$$)' $a.txt; } | sort)
FORCE := false
...
install-gotham: | all clean
	...
	@/usr/bin/rsync -rR --inplace --partial --delete --force ${allfiles} /smb$e
	@#echo Rsyncing to gotham PTS...
	@#/usr/bin/rsync -aR --inplace --partial --delete --force ${allfiles} /smb$f
